<?php
    ...
    class cbreeze {
            ...
            if ($this->conn != NULL) {
                try {
                    ...
                    $this->conn->exec("CREATE TABLE IF NOT EXISTS activities (
                        ...
                        FOREIGN KEY(nameFK) REFERENCES users(name),
                        cardIDFK INT NOT NULL,
                        FOREIGN KEY(cardIDFK) REFERENCES cards(cardID),
                        timestamp DATETIME
                        ) COLLATE utf8_unicode_ci");
        ...
        */
        ...
        /**
         * @param: $cardID- the card ID that was updated, $userID - the user that made a change, $actiondata - JSON arr of data
         * @return: null
         *
         * {
        ...
        public function addActivity($cardID, $userID, $actiondata){
            if ($this->conn != NULL){
                ...
                //we want to generate a time stamp
        ...
        /**
         * @param: $newpriority int generated by the frontend of the new priority, $cardID - ID of the card that changed
         * @return: null
         */
        public function changePriority($cardID, $newpriority, $userID, $oldpriority){
            //we need to simply change the priority in the db
            if ($this->conn != NULL) {
				$query = $this->conn->prepare("UPDATE `cards` SET `priority`='$newpriority' WHERE `cardID`='$cardID'");
				$query->execute();
        ...
        /**
         * @param:  $cardID of the card, $newcolumnID - column to where the card is moved to
         * @return: null
         */
        public function changeColumn($cardID, $newcolumnID, $userID, $oldcolumnID){
            if ($this->conn != NULL) {
